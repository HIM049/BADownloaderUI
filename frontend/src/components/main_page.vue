...
function creatVideoList() {
  status.loading = true
  tipsMsg.loading = "正在生成列表"
  MakeAndSaveList(parms.favListID, 0, false).then(result => {
    if (result != null) {
      LogError(result)
    } else {
      LogInfo("OK!")
...
<template>
  ...
  <div id="fav-input" class="fav-input">
    ...
    <div class="loading-tips" v-show="status.loading">
      <p>{{ tipsMsg.loading }}</p>
      <loadingAnimation />
    </div>
